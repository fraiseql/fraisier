...
class TestHistoryCommand:
    ...
    def test_scaffold_diff_command(self):
        """Test scaffold-diff command output."""
        runner = CliRunner()
        ...
        # Mock compute_scaffold_diff
        mock_diffs = [
            FileDiff(
                generated_path="sudoers",
                installed_path=Path("/etc/sudoers.d/testproject"),
                status="match",
            ),
            FileDiff(
        ...
        with (
            patch(
                "fraisier.scaffold.diff.compute_scaffold_diff", return_value=mock_diffs
            ),
            patch("fraisier.cli.scaffold.require_config", return_value=config),
        ):
            result = runner.invoke(main, ["scaffold-diff"])

            assert result.exit_code == 1  # Differences found
            assert "✓ sudoers" in result.output
            assert "✗ systemd/test.service" in result.output
            assert "Summary: 1/2 files differ" in result.output
