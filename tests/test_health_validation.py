...
class TestDeployHealthPoller:
    """Test deploy-time health polling integration."""
    ...
    def test_poll_succeeds_immediately(self):
        ...
        poller = DeployHealthPoller(
            services={"api": "http://localhost:4001"},
            health_config=cfg,
        )

        healthy_result = AggregateHealthResult(status="healthy")

        with (
            mock_patch.object(
                poller._checker, "check_all", return_value=healthy_result
            ),
            mock_patch("time.sleep"),
        ):
            success, result = poller.wait_until_healthy()

        assert success is True
        assert result.status == "healthy"

    def test_poll_retries_until_healthy(self):
        from unittest.mock import patch as mock_patch
        ...
        unhealthy = AggregateHealthResult(status="unhealthy")
        healthy = AggregateHealthResult(status="healthy")
        call_count = [0]
        ...
        with (
            mock_patch.object(poller._checker, "check_all", side_effect=side_effect),
            mock_patch("time.sleep"),
            mock_patch("time.time") as mock_time,
        ):
            # Simulate time progressing but not exceeding timeout
            mock_time.side_effect = [0, 1, 2, 3, 4, 5]
            success, _result = poller.wait_until_healthy()
    ...
    def test_poll_times_out(self):
        ...
        time_values = iter(range(0, 100))

        with (
            mock_patch.object(poller._checker, "check_all", return_value=unhealthy),
            mock_patch("time.sleep"),
            mock_patch("time.time", side_effect=time_values),
        ):
            success, result = poller.wait_until_healthy()
    ...
    def test_uses_deployment_timeout_override(self):
        from fraisier.config import HealthConfig
        from fraisier.health_check import DeployHealthPoller

        cfg = HealthConfig(startup_timeout_seconds=120)
        poller = DeployHealthPoller(
            services={"api": "http://localhost:4001"},
            health_config=cfg,
            timeout_override=60,
        )
        assert poller.timeout == 60

    def test_uses_config_poll_interval(self):
        from fraisier.config import HealthConfig
        from fraisier.health_check import DeployHealthPoller

        cfg = HealthConfig(deploy_poll_interval_seconds=10)
        poller = DeployHealthPoller(
            services={"api": "http://localhost:4001"},
            health_config=cfg,
        )
