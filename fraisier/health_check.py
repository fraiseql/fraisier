...
class DeployHealthPoller:
    """Poll health checks after deploy until all services are healthy or timeout."""
    ...
    def __init__(
        ...
        self._checker = AggregateHealthChecker(
            services=services,
            health_config=health_config,
        )
        self.timeout = timeout_override or health_config.startup_timeout_seconds
        self.poll_interval = health_config.deploy_poll_interval_seconds
        self.logger = logging.getLogger(__name__)

    def wait_until_healthy(self) -> tuple[bool, AggregateHealthResult]:
        """Poll until all services healthy or timeout exceeded.

        Returns:
            (success, last_result) tuple.
        """
        start = time.time()
        last_result = AggregateHealthResult(status="unhealthy")

        while True:
            last_result = self._checker.check_all()
            if last_result.status == "healthy":
                return True, last_result

            elapsed = time.time() - start
            if elapsed >= self.timeout:
                self.logger.warning(f"Health poll timed out after {self.timeout}s")
