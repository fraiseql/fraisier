...
def trigger_deploy(
    ...
    try:
        ...
        if follow:
            ...
            # outcome must never read as a success. The deploy unit can be killed
            # mid-run (its own socket restarted under it, #349), which closes the
            # connection with nothing written on it; reporting that as "triggered
            # successfully" is how a nightly restore skipped for a day while
            # systemd recorded ExecMainStatus=0.
