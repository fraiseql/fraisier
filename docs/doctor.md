...
| `post_migrate_check_alter_safe` | no `live-drift` gate's DDL tree drops a column with `ALTER TABLE`, which the gate would report as critical drift on a correct database | no | declare the table's final shape in its `CREATE TABLE` and move the drop to a migration — see [when `live-drift` reports drift that is not drift](deployment-guide.md#when-live-drift-reports-drift-that-is-not-drift-407) |
| `post_migrate_check_constraint_coverage` | every `live-drift` gate running `on_critical: fail` escalates `missing_constraint`, which confiture grades a *warning* — so without it the deploy that drops a foreign key still passes | no | add `escalate: [missing_constraint]` to `post_migrate_check` — see [a lost constraint is a warning](deployment-guide.md#a-lost-constraint-is-a-warning-412) |
